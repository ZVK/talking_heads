...
from network.blocks import ResBlockDown, SelfAttention, ResBlock, ResBlockD, ResBlockUp, Padding
import math
import sys
...
class Embedder(torch.nn.Module):
    def __init__(self, in_height):
        ...
        self.sum_pooling = torch.nn.AdaptiveMaxPool2d((1, 1))  # out 512*1*1
...
class Generator(torch.nn.Module):
    ...
    def __init__(self, in_height, finetuning=False, e_finetuning=None):
        ...
        self.in4 = torch.nn.InstanceNorm2d(512, affine=True)

        self.resDown5 = ResBlockDown(512, 512)  # out 512*8*8
        self.in5 = torch.nn.InstanceNorm2d(512, affine=True)

        self.resDown6 = ResBlockDown(512, 512)  # out 512*4*4
        self.in6 = torch.nn.InstanceNorm2d(512, affine=True)
        ...
        self.resUp6 = ResBlockUp(64, 3, out_size=(in_height, in_height), scale=None, conv_size=9,
                                 padding_size=4)  # out 3*224*224

        self.p = torch.nn.Parameter(torch.rand(self.P_LEN, 512).normal_(0.0, 0.02))

        self.finetuning = finetuning
        self.psi = torch.nn.Parameter(torch.rand(self.P_LEN, 1))
        self.e_finetuning = e_finetuning

    def finetuning_init(self):
        if self.finetuning:
            self.psi = torch.nn.Parameter(torch.mm(self.p, self.e_finetuning.mean(dim=0)))
...
class Discriminator(torch.nn.Module):
    def __init__(self, num_videos, finetuning=False, e_finetuning=None):
        ...
        self.sum_pooling = torch.nn.AdaptiveAvgPool2d((1, 1))  # out 512*1*1

        self.W_i = torch.nn.Parameter(torch.rand(512, num_videos))
        self.w_0 = torch.nn.Parameter(torch.randn(512, 1))
        self.b = torch.nn.Parameter(torch.randn(1))

        self.finetuning = finetuning
        self.e_finetuning = e_finetuning
        self.w_prime = torch.nn.Parameter(torch.randn(512, 1))

    def finetuning_init(self):
        if self.finetuning:
            self.w_prime = torch.nn.Parameter(self.w_0 + self.e_finetuning.mean(dim=0))
...
class Cropped_VGG19(torch.nn.Module):
    def __init__(self):
        super(Cropped_VGG19, self).__init__()

        self.conv1_1 = torch.nn.Conv2d(3, 64, 3)
        ...
        self.conv3_3 = torch.nn.Conv2d(256, 256, 3)
        self.conv4_1 = torch.nn.Conv2d(256, 512, 3)
        self.conv4_2 = torch.nn.Conv2d(512, 512, 3)
        self.conv4_3 = torch.nn.Conv2d(512, 512, 3)
        self.conv5_1 = torch.nn.Conv2d(512, 512, 3)
        # self.conv5_2 = torch.nn.Conv2d(512,512,3)
        # self.conv5_3 = torch.nn.Conv2d(512,512,3)
